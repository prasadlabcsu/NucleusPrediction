function [PER_EXP_H] = PER_EXP_VAR_W_HAV(Height,M_Inter)
    ...
    for k = 2:height(Height.Coefficients)
        ...
        if ~contains(Var,':')
            ...
            if Var < 4
                PER_EXP_H(length(PER_EXP_H)+1) = table2array(Height.Coefficients(k,1))*(1/(length(Height.Fitted) - 1))*sum(M_Inter(:,627 + Var).*Height.Fitted);
            else
                Var = Var - 3;
